  ...
  if (auto *Block = dynamic_cast<BlockStmt *>(S)) {
    ...
    if (!soulName.empty() && ShapeMap.count(soulName)) {
      ...
      if (isNamed) {
        ...
        // 1. Verify duplicates in deconstruction list
        ...
        // 2. Verify all specified fields exist in shape
