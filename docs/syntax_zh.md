...
| `auto p = Person(x=1, base, ..)` | 目标状态重组 (Target Elision) | 支持在列表初始化（无论是具名还是按位置）时，使用 `base, ..` 逗号分隔语法。编译器会自动从目标对象（继承源）中捕获对应的字段数据，用于填补当前实例化的结构体中缺失的值，这是局部无损复制和实体状态更新的神兵利器。注意，具名初始化下 `..` 可置于任意位置（如 `Person(.., base, x=1)`）。 |
