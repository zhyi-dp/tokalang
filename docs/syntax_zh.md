...
| `auto Point(y = val, ..) = p` | 具名解构声明 | **铁律**：Toka 支持在 `auto` 解构声明中使用具名绑定。未列出的字段若无默认值，必须显式以 `..` 省略，否则抛出 `E042C`。对非结构体（如 Tagged Enum）进行具名解构抛出 `E0421`。位置与具名形式不可混用，否则抛出 `E0105`。<br>**指针引用**：若绑定受控资源，必须写为 `y = &val` 的形式，否则触发资源复制禁令。 |
| `match p { Point(y = val, ..) => ... }` | 具名模式匹配 | **铁律**：与具名解构一致，在 `match` 分支中匹配 Shape 具名结构。支持使用 `..` 缺省忽略未提及的字段。位置与具名形式同样禁止混用（`E0105`）。 |
