...
| ~~`auto p = Point(1, 2)`~~ | ~~列表初始化，按位置~~ | ~~铁律：可以不具名按位置初始化,且不推荐使用这种方式初始化，除非你非常确定字段的顺序不会改变~~ <br> **【已废除】实例化结构体按位置顺序初始化已彻底废除，强行拦截抛出 `E042A`。** |
...
| ~~`shape Pair(i32, i32)`~~ <br> **已彻底废除** | ~~元组 (Tuple)~~ <br> 匿名具名记录 (Anonymous Record) | ~~匿名结构体，通过 `.0`, `.1` 访问~~ <br> **【已废除】Toka 已彻底废除元组。现推荐使用轻量且可推导的匿名键值对记录 `(x = 10, y = 20)`，通过 `.x`，`.y` 访问以彻底平替元组。** |
| `shape Res(Ok(T) \| Err)` | 枚举 (Enum) | `\|` 分隔变体，支持关联数据 (Tagged Union) |
| ~~`shape IntOrFloat(as i32 \| as f32)`~~ <br> **已彻底废除** | ~~联合 (Union)~~ <br> 裸联合 (Bare Union) | ~~内存重叠，使用 `as` 关键字~~ <br> **【已废除】为了 100% Zero UB，Toka 已彻底废除并剥离裸 Union (Bare Union)！** |
...
| ~~`tuple.0`~~ <br> **已彻底废除** | ~~元组访问~~ <br> 匿名记录访问 | ~~访问元组元素~~ <br> **【已废除】元组已废除。** 现用于访问轻量匿名具名记录的字段值。 |
