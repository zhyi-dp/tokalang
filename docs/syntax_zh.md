...
| **nullptr 和 none** | | |
| `nullptr` | 空指针的字面值 | 仅用于指针变量 |
| `none` | 空值的字面值 |  仅用于实体变量 |
| `auto x: i32? = none` | x是可空类型变量 | 实体变量可空有类型右侧的`?`表达 |
| `auto nul *x: i32 = nullptr` | *x是可空指针变量 |  指针本身可空有声明指针的前置 `nul` 关键字表达|
| `auto nul ^#node#: Node? = nullptr` | `^node` 是一个可空可重绑定的指针变量，他指向可变可空的 `Node` 实体 |  多个维度多个符号的组合表达 |
| `if x == none` 或者 `if *ptr == nullptr` | 检查指针或实体是否为空 |  
...
| `[T]` | 动态数组 (Vec) | 未知长度的连续数组，必定在堆上分配，他不能独立存在，必须是指针形态 如 `*p: [i32]`，解引用的 `p` 就是 `[i32]` 类型；这个设计最大的作用是 动态连续数组不再是原始指针的专属用法 |
| `auto arr: [i32; 3] = [1, 2, 3]` | 静态数组实例化 | 使用圆括号和位置参数初始化，如果声明为`auto arr#` 则可以通过`arr`下标`arr[i]`来修改任何一个元素，对于动态`[T]`同理 |
| `auto ^darr: [i32] = new []i32(1, 2, 3)` | 动态数组实例化,长度根据初始化列表的数量推断 | 使用圆括号和位置参数初始化，原始指针时使用 `alloc` 关键字 |
| `auto ^darr: [i32] = new [3]i32(0)` | 动态数组实例化，指定长度和默认值(广播) | 此时初始化列表中只允许存在一个值，原始指针时使用 `alloc` 关键字 |
