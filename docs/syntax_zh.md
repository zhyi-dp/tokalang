...
| `auto Point(y = val, ..) = p` | 具名解构声明 | **铁律**：Toka 支持在 `auto` 解构声明中使用具名绑定。未列出的字段若无默认值，必须显式以 `..` 省略，否则抛出 `E042C`。对非结构体（如元组）进行具名解构抛出 `E0421`。位置与具名形式不可混用，否则抛出 `E0105`。<br>**指针引用**：若绑定受控资源，必须写为 `y = &val` 的形式，否则触发资源复制禁令。 |
...
| `auto Tuple(x = val, ..)` (对元组进行具名解构) | `auto Tuple(val, _)` | **铁律 (E0421: 元组非结构体禁令)**：元组（Tuple）没有具名字段，仅允许使用位置解构。严禁对元组使用具名解构，否则会抛出 `E0421`。 |
