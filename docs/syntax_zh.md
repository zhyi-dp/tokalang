...
| `auto p = Point(x=1, ..)` | 具名默认值省略 | 若成员定义了默认值，可在具名初始化列表的**最末尾**使用 `..` 显式省略这些字段。(若缺省字段没有定义默认值则编译报错) |
| `auto p = Point(1, .., 3)` | 按位置弹性省略 | 按位置初始化时，`..` **不用非得放在末尾**！你可以放在中间或开头（如 `Person(.., 99.0)`），编译器会自动推演两端的数量并用默认值填充中间被挤压的字段。但一场初始化**最多只能有一个** `..`。 |
