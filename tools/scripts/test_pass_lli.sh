...
# Attempt to find LLVM 20 lli path dynamically via brew
if command -v brew >/dev/null 2>&1; then
    LLVM_LLI="$(brew --prefix llvm@20 2>/dev/null || brew --prefix llvm 2>/dev/null)/bin/lli"
fi

# Fallback paths
if [ -z "$LLVM_LLI" ] || [ ! -f "$LLVM_LLI" ]; then
    if [ -f "/opt/homebrew/opt/llvm@20/bin/lli" ]; then
        LLVM_LLI="/opt/homebrew/opt/llvm@20/bin/lli"
    elif [ -f "/usr/local/opt/llvm@20/bin/lli" ]; then
        LLVM_LLI="/usr/local/opt/llvm@20/bin/lli"
    else
        LLVM_LLI="lli-20"
    fi
fi

if ! command -v "$LLVM_LLI" >/dev/null 2>&1 && [ ! -f "$LLVM_LLI" ]; then
    echo "Error: LLVM 20 lli not found. Cannot run JIT tests."
    exit 1
fi
